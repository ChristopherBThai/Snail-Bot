...
module.exports = new CommandInterface({
	...
	execute: async function () {
		...
		if (
			!(
				this.global.hasRoles(this.msg.member, this.config.roles.role_change) ||
		...
		if (this.msg.args.length < 2) {
			await this.error(
				', the correct command arguments are `changerole {hexcode} {role name}`'
			);
			return;
		}
		...
		if (hexcode.length !== 6) {
			await this.error(
				', Invalid hexcode! It should look something like this: `#ABCDEF`'
			);
			return;
		}
		...
		if (roleName.length > 100) {
			await this.error(
				', The role name is too long! Must be under 100 characters.'
			);
			return;
		}
		...
		if (r < 75 && r > 43 && g < 75 && g > 43 && b < 75 && b > 43) {
			await this.error(
				", This color is too close to Discord's dark theme! No invisible roles!"
			);
			return;
		}
		...
		try {
			...
			if (user && user.role && user.role._id) {
				userRole = await updateRole.bind(this)(
					user.role._id,
					intcode,
					roleName
				);
		...
		await this.log(
			`${this.msg.author.mention} changed their role to **${
				userRole.name
			}** #${userRole.color.toString(16).padStart(6, '0')}`
		);
...
async function updateRole(id, hexcode, roleName) {
	...
	await existingRole.edit(
		{ name: roleName, color: hexcode },
		this.msg.author.id + ' edited their role'
	);

	// Add role to member
	await this.msg.channel.guild.addMemberRole(
		this.msg.member.id,
		id,
		this.msg.author.id + ' edited their role'
	);
...
async function addRole(hexcode, roleName) {
	...
	// Change position
	let upperPosition = this.msg.channel.guild.roles.get(
		this.config.roles.role_upper
	).position;
