...
module.exports = new Command({
	...
	execute: async function () {
		...
		if (!QuestList) {
			await this.error("I don't have a Quest List module. Did Wifu forget to delete this command or was the module deleted?");
			return;
		}
		...
		switch (subcommand) {
			...
			case "notifyclear": {
				...
				await this.send(`I have cleared the ${type == "all" ? "quest" : QUEST_DATA[type].name.toLowerCase()} list!`);
			...
			case "remove": {
				...
				switch (type) {
					...
					case "action": type = "emoteBy"; break;
					default: {
						await this.error("that is not a valid quest type! The valid types are `all`, `cookie`, `pray`, `curse`, and `action` and the command is `snail ql remove [type] {@users...}`");
						return;
					};
				}
			...
			case "setmax": {
				...
				if (!amount || amount < 1) {
					await this.error(`${this.message.args[0]} is not a valid number! Please select a number greater than 0.`);
					return;
				}
			...
			case "view": {
				...
				let text = '';
				for (const [type, data] of Object.entries(QUEST_DATA).filter(([type]) => QUESTS_GROUPED_BY_TYPE[type])) {
