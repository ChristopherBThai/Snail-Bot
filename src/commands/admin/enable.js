...
module.exports = new CommandInterface({
    ...
    execute: async function () {
        ...
        if (enabledCommands.includes("all")) {
            enabledCommands = Object.values(this.bot.commands).map(command => command.alias[0]);
        } else {
            enabledCommands = enabledCommands.map(command => this.bot.commands[command].alias[0]);
        }
