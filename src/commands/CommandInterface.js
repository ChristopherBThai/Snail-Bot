...
module.exports = class CommandInterface {
	...
	async execute(params) {
		await params.message.channel.sendTyping();

		if (this.auth?.(params.message.member) ?? true) {
			// Staff are not bound by the chains of cooldowns >:)
			if (!isStaff(params.message.member)) {
				const commandName = this.alias[0];
				const key = `${params.message.author.id}_${commandName}`;
