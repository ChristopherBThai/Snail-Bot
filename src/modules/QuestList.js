...
module.exports = class QuestList extends require("./Module") {
    ...
    async onOwOCommand({ command, message }) {
        ...
        const newQuests = usersQuests.filter(newQuest => {
            return Object.keys(QUEST_DATA).includes(newQuest.type)				// Check that it is a type of quest that can be added to the list
                ...
                && !this.quests.some(quest => areSameQuest(newQuest, quest));	// Check that the quest is not already on the list
        });

        // Add the new quests to the list
        this.quests.push(...newQuests.map(quest => { return { ...quest, added: Date.now() } }));
        ...
        if (newQuests.length == 0) {
            ...
            if (difference > MESSAGE_TIMEOUT) {
                await ephemeralResponse(message, `🚫 **|** <@${USER_ID}>, You don't have any new quests to add to the list! Only unlocked Cookie, Pray, Curse, and Action quests can be added!`, MESSAGE_TIMEOUT);
                this.warnings[USER_ID] = now;
            }
        } else {
            let text = `<@${USER_ID}>, I have added your quest(s) to the list!\n` + newQuests.map(quest => {
    ...
    async update() {
        ...
        this.quests = this.quests.map(quest => { return { ...UPDATED_QUESTS.find(updatedQuest => areSameQuest(updatedQuest, quest)), added: quest.added } }).filter(quest => quest?.locked == 0);

        // Update quest list in database for presistence between restarts
        await this.bot.snail_db.Quest.deleteMany({});
        await this.bot.snail_db.Quest.insertMany(this.quests.map(({ discordID, claimed, added }) => { return { discordID, claimed, added } }));
        ...
        embed.fields = Object.entries(QUEST_DATA).filter(([type]) => QUESTS_GROUPED_BY_TYPE[type]).map(([type, data]) => {
            ...
            for (const USER in QUESTS_GROUPED_BY_USER) {
                ...
                const QUEST_STRING = `${counts} \`${getName(this.bot.guilds.get(this.bot.config.guild)?.members.get(USER))}\` <@${USER}>\n`;
    ...
    async onButtonPress(interaction) {
        const { type, data: { custom_id }, member: { id: MEMBER_ID } } = interaction;
        ...
        switch (custom_id) {
            ...
            case "questlist_toggle_reminders": {
                ...
                await this.bot.snail_db.User.updateOne({ _id: MEMBER_ID }, { reminders: { luck: { enabled: !(enabled ?? false) } } }, { upsert: true });

                await interaction.createMessage({ content: `You have ${enabled ? "disabled" : "enabled"} pray/curse reminders.`, flags: 1 << 6 });
                break;
            }
        }
    }
