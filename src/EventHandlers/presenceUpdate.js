	...
	async handle (other, oldPresence) {
		if (other.user.id == this.bot.config["owo-bot"]) {
			if (other.status == "online" && oldPresence.status != "online") {
				...
				await this.setLimit(this.slowmode, `The bot is down right now, it will take some time to come back online! If you have any questions please post it all in one message\n*Setting slowmode to ${this.slowmode} seconds*`);
			}
		}
	}
