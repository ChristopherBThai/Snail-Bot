...
module.exports = class MessageCreateHandler {
	...
	async manageQuestHelp(msg) {
		...
		const new_quests = result.filter(newQuest => {
			return Object.keys(QUEST_DATA).includes(newQuest.type)				// Check that it is a type of quest that can be added to the list
				...
				&& !QUEST_LIST.some(quest => areSameQuest(newQuest, quest));	// Check that the quest is not already on the list
		});

		QUEST_LIST.push(...new_quests);		// Add the new quests to the list
	...
	async updateQuestListMessage() {
		...
		QUEST_LIST = UPDATED_QUESTS.filter(updatedQuest => QUEST_LIST.some(quest => areSameQuest(quest, updatedQuest) && updatedQuest.locked == 0));

		const GROUPED_QUESTS = QUEST_LIST.reduce((groups, quest) => {
		...
		if (MESSAGES_SINCE_LAST_POST >= MESSAGES_UNTIL_REPOST || !QUEST_LIST_MESSAGE) {
