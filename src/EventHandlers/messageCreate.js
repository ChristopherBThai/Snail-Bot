...
module.exports = class MessageCreateHandler {
	...
	async checkModMention(msg) {
		if (msg.mentions && msg.mentions.length > 0) {
			if (
				global.hasRoles(msg.member, this.bot.config.roles.mods) ||
			...
			msg.mentions.forEach(async (mention) => {
				...
				if (
					global.hasRoles(member, this.bot.config.roles.mods) ||
					...
					if (
						...
						let warnMsg = await msg.channel.createMessage(
							`⚠️ **|** ${msg.author.mention}, please refrain from tagging \`offline\` or \`do not disturb\` helpers/mods!`
						);
						...
						await this.bot.createMessage(
							...
							`⚠️ **|** ${msg.author.mention} tagged ${member.username}#${member.discriminator} in ${msg.channel.mention} ${link}`
						);
					}
					...
					// Tags in spam channel
						...
						if (user.friends && user.friends.has(msg.author.id)) return;
						let warnMsg = await msg.channel.createMessage(
							`⚠️ **|** ${msg.author.mention}, please refrain from tagging helper/mods in spam channels!`
						);
						const link = `https://discordapp.com/channels/${msg.channel.guild.id}/${msg.channel.id}/${warnMsg.id}`;
						await this.bot.createMessage(
							this.bot.config.channels.log,
